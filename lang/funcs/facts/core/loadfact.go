...
// LoadScale factor scales the output from sysinfo to the correct float value.
const LoadScale = 65536 // XXX: is this correct or should it be 65535?
...
func (obj *LoadFact) Info() *facts.Info {
	return &facts.Info{
		Output: types.NewType("float"), // XXX: switch to a struct instead
	}
}
...
func (obj *LoadFact) Stream() error {
	...
	for {
		...
		_, _, _ = x1, x5, x15 // XXX: temporary until it's a struct
		select {
		case obj.init.Output <- &types.FloatValue{ // XXX: just for testing
			V: x1,
		}:
		//case obj.init.Output <- &types.ListValue{ // TODO: would a struct be better?
		//	T: types.NewType("[]float"),
		//	V: []types.Value{
		//		&types.FloatValue{V: x1},
		//		&types.FloatValue{V: x5},
		//		&types.FloatValue{V: x15},
		//	},
		//}:
		case <-obj.closeChan:
			return nil
